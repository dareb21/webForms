            ...
            <div class="flex flex-col items-center">
                ...
                <div class="w-full h-full mt-6 overflow-x-auto">
                    ...
                    <div class="w-full flex flex-col items-start pt-3">
                        <div class="flex gap-x-4 flex-wrap py-4">
                            ...
                            <form action="{{ route('adminEvaluationSearch') }}" method="GET">
                                <input type="text" name="adminSearch" id="adminSearch"
                                    value="{{ request('adminSearch') }}" class="shadow-md border border-gray-200">
                                <select name="adminSearchSelect" id="adminSearchSelect"
                                    class="shadow-md border border-gray-200">
                                    <option value="revision"
                                        {{ request('adminSearchSelect') == 'revision' ? 'selected' : '' }}>Revisión</option>
                                    <option value="autor" {{ request('adminSearchSelect') == 'autor' ? 'selected' : '' }}>
                                        Autor</option>
                                    <option value="fechaInicio"
                                        {{ request('adminSearchSelect') == 'fechaInicio' ? 'selected' : '' }}>Fecha Inicio
                                    </option>
                                </select>
                                <button type="submit"
                    ...
                    <table class="table-auto border border-gray-400 w-full min-w-[600px] text-left">
                        <thead>
                            <tr>
                                <th class="border border-gray-400 px-4 py-2 text-center bg-blue-600 text-white md:w-60">
                                    Revisión</th>
                                <th class="border border-gray-400 px-4 py-2 text-center bg-blue-600 text-white">Fecha de
                                    Inicio</th>
                                <th class="border border-gray-400 px-4 py-2 text-center bg-blue-600 text-white">Fecha de
                                    Cierre</th>
                ...
                <div class="p-6">
                    <a href="{{ route('adminNewEvaluation') }}"
