    ...
    <div class="flex-1 ml-0 md:ml-64 p-4 bg-gray-200 min-h-[calc(100vh-4rem)] overflow-auto">
        <div class="bg-white rounded-xl shadow-lg p-6 w-full h-full relative pb-24">
            ...
            <form action="{{ route('adminUpdateOrReuse', ['surveyId' => $surveyId]) }}" method="POST">
                ...
                <div class="flex flex-col md:flex-row justify-center gap-6 py-4 flex-wrap">
                    ...
                    <div class="flex flex-col gap-4 w-full md:w-auto">
                        <div class="flex flex-col md:flex-row items-center gap-2">
                            ...
                            <input type="text" name="evaluationName"
                                value="{{ $evaluacion }}"class="shadow-md border border-gray-200 w-full md:w-auto">
                        </div>
                        <div class="flex flex-col md:flex-row items-center gap-2">
                    ...
                    @foreach ($questionGroups as $group)
                        ...
                        <div class="flex flex-col gap-4 py-4">
                            ...
                            <div class="flex flex-col md:flex-row items-center gap-2 w-full">
                                ...
                                <input type="text"
                                    name="options[{{ $group->id }}][{{ $groupOptions[0]->id ?? 'new1' }}]" id=""
                                    value="{{ $groupOptions[0]->option ?? '' }}"
                                    class="shadow-md border border-gray-200 flex-1 w-full">
                                <label for="g{{ $group->id }}c1">Calificación</label>
                                <input type="number" name="cal[{{ $group->id }}][c1]" id="g{{ $group->id }}c1"
                                    value="{{ $groupOptions[0]->calification ?? '' }}"
                                    class="shadow-md border border-gray-200 w-10">
                            </div>
                            <div class="flex flex-col md:flex-row items-center gap-2 w-full">
                                <label class="whitespace-nowrap">Pregunta 2 -</label>
                                <input type="text"
                                    name="options[{{ $group->id }}][{{ $groupOptions[1]->id ?? 'new1' }}]"
                                    id="" value="{{ $groupOptions[1]->option ?? '' }}"
                                    class="shadow-md border border-gray-200 flex-1 w-full">
                                <label for="g{{ $group->id }}c2">Calificación</label>
                                <input type="number" name="cal[{{ $group->id }}][c2]" id="g{{ $group->id }}c2"
                                    value="{{ $groupOptions[1]->calification ?? '' }}"
                                    class="shadow-md border border-gray-200 w-10">
                            </div>
                        </div>
                    @endforeach
                ...
                <!-- Botones -->
                <div
                    class="w-full flex flex-col items-center md:flex-row md:justify-center gap-4 py-4 bg-white rounded-b-xl mt-8">
                    <a id="agregar-grupo"
                        class="w-auto h-auto text-center bg-blue-600 hover:bg-orange-500 text-white font-bold py-1 px-4 rounded hover:cursor-pointer">
                        AGREGAR
                    </a>
                    <a id="borrar-grupo"
                        class="w-auto h-auto bg-blue-600 hover:bg-orange-500 text-white font-bold py-1 px-4 rounded hover:cursor-pointer">
                        BORRAR
                    </a>
                </div>
                ...
                <div class="w-full flex flex-col lg:flex-row justify-center gap-4 mt-8">
                    ...
                    <div
                        class="text-center w-full lg:w-auto flex flex-col items-center md:flex-row md:justify-center gap-1 py-4 bg-white rounded-b-xl">
                        <button type="submit" name="btn"
                            value="update"class="w-30 bg-blue-600 hover:cursor-pointer hover:bg-orange-500 text-white font-bold py-1 px-4 rounded">
                            GUARDAR
                            </a>
                            <input type="hidden" name="survey_id" value="{{ $surveyId }}">
                            <button type="submit" name="btn" value="reuse"
                                class="w-30 bg-blue-600 hover:cursor-pointer hover:bg-orange-500 text-white font-bold py-1 px-4 rounded">
                                REUTILIZAR
                            </button>
            </form>
            <a href="{{ route('enableEvaluation', ['surveyId' => $surveyId]) }}"
                class="w-30 bg-blue-600 hover:bg-orange-500 text-white font-bold py-1 px-4 rounded">
                ACTIVAR
            </a>
        </div>
        ...
        <div
            class="text-center w-full lg:w-auto flex flex-col items-center md:flex-row md:justify-center gap-1 py-4 bg-white rounded-b-xl">
            <a href="{{ route('unableEvaluation', ['surveyId' => $surveyId]) }}"
                class="w-30 bg-red-600 hover:bg-blue-700 text-white font-bold py-1 px-4 rounded">
                DESACTIVAR
            </a>
            <a href="{{ route('adminDelete', ['surveyId' => $surveyId]) }}"
                class="w-30 bg-red-600 hover:bg-orange-500 text-white font-bold py-1 px-4 rounded">
                ELIMINAR
            </a>
