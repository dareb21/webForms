    ...
    <div class="bg-white rounded-xl shadow-lg p-6 w-full h-full">
        <div class="flex flex-col items-center">
            <div class="bg-white p-4 text-center text-2xl font-bold">
                <h1 class="uppercase">
                    EVALUACIONES DE <span class="italic">{{ request('courses') }}</span>
                </h1>
            ...
            <div class="w-full h-full mt-6 overflow-x-auto">
                <table class="table-auto border border-gray-400 w-full min-w-[600px] text-left">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="border border-gray-400 px-4 py-2 text-center">{{ $results['nameStudent'] }}</td>
  ...
  function directorAnswers() {
    return {
      ...
      fetchAnswers(submitId) {
        this.open = true;
        this.loading = true;
        fetch(`/directorViewAnswer/${submitId}`)
          .then(res => res.json())
          .then(data => {
            this.observation = '';
