...
@section('content')
    ...
    <main class="flex-1 ml-0 md:ml-80 p-6 bg-gray-200 min-h-[calc(100vh-4rem)] overflow-auto">
        <section class="bg-white rounded-xl shadow-lg p-6">
            ...
            <form action="{{ route('studentSubmit', ['courseId' => $coursesId]) }}" method="POST"
                        ...
                        <tbody>
                            @foreach ($data as $groupName => $group)
                                @foreach ($group as $index => $item)
                                    <tr>
                                        @if ($index === 0)
                                            <th class="border border-gray-400 text-center bg-blue-600 text-white w-24 md:w-36 px-4 py-2 align-middle"
                                                rowspan="{{ count($group) }}" scope="rowgroup">
                                                Indicador {{ $groupName }}
                                            </th>
                                        @endif
                                        <td class="border border-gray-400 px-4 py-2" colspan="4">
                                            <div class="flex flex-col divide-y divide-gray-300">
                                                <div class="flex justify-between items-center py-2">
                                                    <span
                                                        class="font-medium">{{ $item['option1'] ?? 'Pregunta sin texto' }}</span>
                                                    <input type="checkbox" name="option_{{ $item['option1Id'] }}"
                                                        id="option_{{ $item['option1Id'] }}" class="w-5 h-5 text-blue-600"
                                                        data-group="{{ $groupName }}">
                                                </div>

                                                <div class="flex justify-between items-center py-2">
                                                    <span
                                                        class="font-medium">{{ $item['option2'] ?? 'Pregunta sin texto' }}</span>
                                                    <input type="checkbox" name="option_{{ $item['option2Id'] }}"
                                                        id="option_{{ $item['option2Id'] }}" class="w-5 h-5 text-blue-600"
                                                        data-group="{{ $groupName }}">
                                                </div>
                                            </div>
                                        </td>
                ...
                <section class="mt-6">
                    ...
                    <textarea name="observaciones"
                        ...
                        placeholder="Escribe tus observaciones aquí..." aria-label="Observaciones"></textarea>
                </section>
